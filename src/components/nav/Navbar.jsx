...
//   // Lock body scroll when mobile search or city drawer open
//   useEffect(() => {
...
//           {/* ── MOBILE: Search + City icons (left of menu) ── */}
...
//               className="flex items-center gap-1 px-2 py-1.5 text-xs font-medium text-gray-700 border rounded-lg hover:bg-gray-50 active:bg-gray-100 transition-colors"
//               aria-label="Select city"
//             >
//               <MapPin className="w-3.5 h-3.5 text-orange-500 flex-shrink-0" />
//               <span className="max-w-[70px] truncate">{selectedLocation}</span>
//             </button>
//           </div>

//           {/* ── RIGHT SIDE (desktop + mobile shared) ── */}
//           <div className="flex items-center gap-1 lg:gap-2 ml-2 md:ml-2 flex-shrink-0">
//             {/* List Your Business — hidden on mobile */}
//             <button
//               onClick={() =>
//                 window.open(
...
//       {/* ── MOBILE: City Selector Bottom Sheet ── */}
//       {mobileCityOpen && (
...
export default function Navbar() {
  ...
  return (
    <>
      {/* Spacer — accounts for both navbar rows on mobile */}
      ...
      <nav className="w-full bg-white fixed top-0 left-0 right-0 z-50 shadow-sm">
        ...
        <div className="px-3 sm:px-4 md:px-5 lg:px-6 py-2">
          <div className="max-w-[1400px] mx-auto flex items-center gap-1 sm:gap-1.5 lg:gap-2">
            ...
            <div className="flex items-center gap-1 lg:gap-2 ml-2 flex-shrink-0">
              ...
              {/* List Your Business — desktop only */}
              <button
                onClick={() =>
                  window.open(
                    "https://vendor.localtradestreet.com/signin",
              ...
              {/* Nav links — desktop only (xl) */}
              {navLinks.map((link) => (
                <button
                  key={link.path}
                  onClick={() => navigate(link.path)}
                  className="hidden xl:block cursor-pointer text-xs lg:text-sm hover:text-orange-500 px-1 lg:px-2 whitespace-nowrap font-medium text-gray-700 transition-colors duration-200"
                >
                  {link.label}
                </button>
              ))}
        ...
        {/* ── ROW 2: Nav Links — mobile only (below md) ── */}
          ...
          <div className="flex items-center justify-around">
            {navLinks.map((link) => (
              <button
                key={link.path}
                onClick={() => navigate(link.path)}
                className="text-[11px] font-medium text-gray-600 hover:text-orange-500 active:text-orange-600 px-2 py-1 rounded-md transition-colors"
              >
                {link.label}
              </button>
            ))}
            {/* List Business — mobile me bhi dikhega yahan */}
            <button
              onClick={() =>
                window.open(
                  "https://vendor.localtradestreet.com/signin",
