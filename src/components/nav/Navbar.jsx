...
//             <button onClick={()=>{navigate("/about")}} className="cursor-pointer text-sm cursor-pointer hover:text-orange-500 px-2">Contact</button>
//             <button onClick={()=>{navigate("/contact")}} className="cursor-pointer text-sm cursor-pointer hover:text-orange-500 px-2">About</button>
//             <button
...
export default function Navbar() {
  ...
  return (
    <>
          ...
          <div className="relative hidden md:block" ref={dropdownRef}>
            ...
            {dropdownOpen && (
              <div className="absolute top-full left-0 mt-2 bg-white border border-gray-200 rounded-xl shadow-2xl z-[100] w-72 overflow-hidden flex flex-col">
                ...
                <div className="max-h-[300px] overflow-y-auto cursor-pointer" onScroll={handleScroll}>
                  {loading ? (
      ...
      <HamburgerDrawer isOpen={menuOpen} onClose={() => setMenuOpen(false)} />
      {profileOpen && (localStorage.getItem("token") ? (
