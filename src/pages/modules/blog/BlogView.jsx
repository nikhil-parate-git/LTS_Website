...
// pages/modules/blog/BlogView.jsx
import React, { useEffect } from "react";
import { useDispatch, useSelector } from "react-redux";
import { useParams, useNavigate } from "react-router-dom";
...
const PageLoader = () => (
  <div className="flex flex-col items-center justify-center min-h-[60vh] w-full">
    ...
    <p className="mt-4 text-[11px] font-bold text-gray-400 uppercase tracking-[0.2em]">Loading Article...</p>
  </div>
);
...
const BlogView = () => {
  ...
  const { currentBlog, blogDetailLoading, blogDetailError } = useSelector((state) => state.blogs);
  const { categories: apiCategories } = useSelector((state) => state.categories);

  // Resolve cateId from Redux categories using the URL slug
  const resolveCategoryApiId = () => {
    if (!apiCategories || !categorySlug) return null;
  ...
  useEffect(() => {
    if (!blogId) return;
    const categoryApiId = resolveCategoryApiId();
    if (!categoryApiId) return; // wait for categories to load

    dispatch(fetchBlogById({ categoryApiId, blogId }));
    return () => { dispatch(clearBlogDetail()); };
  }, [dispatch, blogId, categorySlug, apiCategories]);

  const handleBack = () => navigate(-1);

  // Strip HTML for meta description
  const plainDescription = currentBlog?.description
    ?.replace(/<[^>]+>/g, "")
    .slice(0, 160)
    .trim();
  ...
  return (
    <div className="min-h-screen bg-[#fcfcfb] font-sans">
      ...
      <Helmet>
        <title>{currentBlog ? `${currentBlog.title} | Local Trade Street` : "Article | Local Trade Street"}</title>
        {plainDescription && <meta name="description" content={plainDescription} />}
        <meta name="robots" content="index, follow" />
        {currentBlog?.title && <meta property="og:title" content={`${currentBlog.title} | Local Trade Street`} />}
        ...
        {currentBlog?.title && <meta name="twitter:title" content={currentBlog.title} />}
      ...
      <nav className="sticky top-0 z-30 bg-white/90 backdrop-blur-sm border-b border-gray-100">
        <div className="w-full mx-auto px-4 py-3 flex items-center justify-between">
          <button onClick={handleBack} className="flex cursor-pointer items-center gap-2 text-sm font-bold text-gray-600 hover:text-orange-500 transition-colors">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M15 19l-7-7 7-7" />
            </svg>
            Back
          </button>
          <span className="text-[10px] font-black uppercase tracking-widest text-gray-400">Article View</span>
        </div>
      </nav>
      ...
      <main className="w-full mx-auto px-4 py-8">
        ...
        {blogDetailError && !blogDetailLoading && (
          <div className="flex flex-col items-center justify-center min-h-[50vh] text-center">
            <p className="text-red-500 font-bold mb-4">{blogDetailError}</p>
            <button onClick={() => window.location.reload()} className="px-8 py-2.5 bg-gray-900 text-white rounded-xl text-xs font-bold hover:bg-orange-600 transition-all shadow-lg">
              Try Again
            </button>
          </div>
        )}
        ...
        {!blogDetailLoading && !blogDetailError && currentBlog && (
          <div className="animate-in fade-in duration-500">
            ...
            <div className="flex items-center gap-3 mb-7 pb-5 border-b border-gray-100">
              ...
              <div>
                <p className="text-sm font-extrabold text-gray-900 leading-tight">{currentBlog.author}</p>
                <div className="flex items-center gap-1 mt-0.5">
                  <svg className="w-3 h-3 text-orange-500" fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                  </svg>
                  <span className="text-[11px] text-orange-500 font-semibold">Verified Author</span>
                </div>
              </div>
            </div>
            ...
            <div className="mb-8 w-full overflow-hidden rounded-2xl border border-gray-100 shadow-sm">
              {currentBlog.image ? (
                <img src={currentBlog.image} alt={currentBlog.title} className="w-full h-[300px] md:h-[500px] object-cover block" />
              ) : (
                <div className="w-full h-48 bg-gray-100 flex items-center justify-center">
                  <span className="text-4xl opacity-20">📄</span>
                </div>
            ...
            {currentBlog.description ? (
              <div className="text-gray-700 leading-relaxed text-base prose prose-sm max-w-none" dangerouslySetInnerHTML={{ __html: currentBlog.description }} />
            ) : (
              <p className="text-gray-400 italic text-sm text-center py-10">No content available for this post.</p>
            )}
            ...
            <div className="mt-5 pb-8">
              <button onClick={handleBack} className="px-8 py-3 bg-gray-900 text-white rounded-xl text-xs font-bold hover:bg-orange-600 transition-all shadow-lg active:scale-95 cursor-pointer">
                ← All Articles
              </button>
            </div>
          </div>
