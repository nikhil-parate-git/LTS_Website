...
import {
  Search, ChevronRight, Home, ArrowLeft, LayoutGrid,
  Star, TrendingUp, Sparkles, ChevronRight as ArrowRight,
  Grid3X3, Loader2,
} from "lucide-react";
...
const COLOR_SCHEMES = [
  { gradient: "from-orange-50 to-amber-50",  border: "border-orange-200",  iconBg: "bg-gradient-to-br from-orange-400 to-amber-300",  accent: "text-orange-600"  },
  { gradient: "from-blue-50 to-indigo-50",   border: "border-blue-200",    iconBg: "bg-gradient-to-br from-blue-500 to-indigo-300",   accent: "text-blue-600"    },
  { gradient: "from-emerald-50 to-teal-50",  border: "border-emerald-200", iconBg: "bg-gradient-to-br from-emerald-500 to-teal-300",  accent: "text-emerald-600" },
  { gradient: "from-purple-50 to-pink-50",   border: "border-purple-200",  iconBg: "bg-gradient-to-br from-purple-500 to-pink-300",   accent: "text-purple-600"  },
];

// ─── SubCategoryCard ───────────────────────────────────────────────────────────
const SubCategoryCard = memo(function SubCategoryCard({ cat, city }) {
  const navigate = useNavigate();

  const handleClick = useCallback(() => {
    if (!cat.catId || !cat.subCatId) {
      console.warn("Missing catId or subCatId", cat);
      return;
    }
    const subSlug = createSlug(cat.title);
    const citySlug = createSlug(city || "india");
    navigate(`/service/${cat.catId}/${cat.subCatId}/${citySlug}/${subSlug}`);
  }, [cat.catId, cat.subCatId, cat.title, city, navigate]);
  ...
  return (
    ...
    >
      ...
      <div className="p-5 pb-3">
        <div className="flex items-start justify-between mb-3">
          <div className={`p-3 ${cat.colorScheme.iconBg} rounded-xl shadow-lg transform group-hover:scale-110 transition-transform duration-300`}>
            <LayoutGrid size={20} className="text-white" />
          </div>
        </div>
        <h2 className={`font-bold text-base sm:text-lg ${cat.colorScheme.accent} leading-tight group-hover:translate-x-1 transition-transform duration-300 mb-2`}>
          {cat.title}
        </h2>
        <div className="flex items-center gap-2 text-xs text-gray-500">
          <TrendingUp size={12} />
        ...
        <ul className="space-y-2 max-h-64 overflow-y-auto">
          {cat.items.map((item, idx) => (
            <li
              ...
              className="group/item relative flex items-center gap-2 text-sm text-gray-700 hover:text-orange-600 cursor-pointer transition-all duration-200 py-1.5 px-2 rounded-lg hover:bg-white/50 hover:shadow-sm"
            >
              <span className="flex-shrink-0 w-1 h-1 rounded-full bg-gradient-to-r from-orange-400 to-amber-400 group-hover/item:scale-125 transition-all duration-200" />
              <span className="flex-1 font-medium group-hover/item:translate-x-1 transition-transform duration-200">
                {item}
              </span>
              <ArrowRight
                size={12}
                className="opacity-0 group-hover/item:opacity-100 transition-all duration-200 transform -translate-x-2 group-hover/item:translate-x-0"
              />
            </li>
          ))}
        </ul>
...
const TopCategoryDetails = () => {
  ...
  const { categoryData, loading, error } = useSelector((state) => state.topcategoryById);
  const { selectedCity } = useSelector((state) => state.location);

  useEffect(() => {
    if (cateId) dispatch(fetchCategoryById(cateId));
    return () => dispatch(resetCategoryData());
  }, [dispatch, cateId]);
  ...
  const { filteredSections, categoryLabel } = useMemo(() => {
    if (!categoryData?.length)
      return { filteredSections: [], categoryLabel: "" };

    const categoryLabel = categoryData[0]?.categoryId?.name || "Category Details";

    const allSections = categoryData.map((item, index) => ({
      id:          item._id,
      catId:       item.categoryId?.cateId,
      subCatId:    item.subCategoryId?.subCateId,
      title:       item.subCategoryId?.name || "Service",
    ...
    <div className="min-h-screen font-sans bg-gradient-to-br from-gray-50 via-white to-gray-50">
      ...
      <div className="sticky top-0 z-10 bg-white/95 backdrop-blur-sm shadow-lg border-b border-gray-100">
        <div className="w-full px-3 sm:px-6 md:px-8 py-3 md:py-4">
          <h1 className="text-center text-base sm:text-xl md:text-2xl font-bold bg-gradient-to-r from-gray-800 to-gray-600 bg-clip-text text-transparent mb-3 mt-5">
            {categoryLabel
              ? `${categoryLabel} — Find Top Businesses In Your City`
              : "Find Top Businesses In Your City"}
          </h1>
          <div className="flex flex-col sm:flex-row gap-2 max-w-3xl mx-auto mt-5 mb-5">
            <div className="flex items-center border-2 border-gray-200 rounded-xl bg-white px-3 py-2 gap-2 sm:flex-1 focus-within:border-orange-400 transition-all duration-200 shadow-sm hover:shadow-md">
      ...
      {/* Breadcrumb */}
      <div className="w-full px-3 sm:px-6 md:px-8 mt-2">
        <nav aria-label="Breadcrumb" className="flex items-center justify-between py-2 sm:py-2.5 gap-2">
          <div className="flex items-center gap-2 overflow-x-auto min-w-0">
            <button
              onClick={goBack}
              aria-label="Go back"
              className="flex items-center justify-center w-7 h-7 sm:w-8 sm:h-8 rounded-full border-2 border-gray-200 bg-white hover:bg-gradient-to-r hover:from-orange-500 hover:to-amber-500 hover:text-white hover:border-transparent transition-all duration-200 shadow-sm shrink-0"
            >
              <ArrowLeft size={13} />
            </button>
            <div className="w-px h-4 bg-gray-200 shrink-0" />
            ...
            <span className="inline-flex items-center gap-1.5 text-[10px] sm:text-[11px] font-semibold text-orange-600 bg-gradient-to-r from-orange-50 to-amber-50 border border-orange-200 rounded-full px-2.5 py-0.5 whitespace-nowrap shadow-sm">
              <span className="w-1.5 h-1.5 rounded-full bg-gradient-to-r from-orange-500 to-amber-500 shrink-0 animate-pulse" />
              <span className="max-w-[150px] truncate">{categoryLabel}</span>
            </span>
          </div>
          <span className="text-[11px] text-gray-600 bg-gray-100 border border-gray-200 rounded-full px-3 py-1 font-medium shadow-sm hidden sm:block shrink-0">
            {filteredSections.length} Sub-Categories
          </span>
        </nav>
      </div>
      ...
      <div className="w-full px-3 sm:px-6 md:px-8 py-6 sm:py-8">
        {filteredSections.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-7">
            {filteredSections.map((cat) => (
              <SubCategoryCard key={cat.id} cat={cat} city={selectedCity} />
            ))}
          </div>
        ) : (
          <div className="text-center py-20 bg-white rounded-3xl border-2 border-dashed border-gray-200 shadow-inner">
            <Grid3X3 size={32} className="text-gray-400 mx-auto mb-4" />
            <p className="text-gray-500 font-medium text-lg">
              No subcategories found matching your search.
            </p>
          </div>
      ...
      <style>{`
        ...
        ul::-webkit-scrollbar-track { background: #f3f4f6; border-radius: 10px; }
        ul::-webkit-scrollbar-thumb { background: #e5e7eb; border-radius: 10px; }
