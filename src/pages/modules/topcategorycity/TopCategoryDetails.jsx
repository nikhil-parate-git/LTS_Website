...
const TopCategoryDetails = () => {
  ...
  const { filteredSections, categoryLabel } = useMemo(() => {
    const rawLabel =
      categoryData?.[0]?.categoryId?.name || slug?.replace(/-/g, " ") || "";

    // Logic to Capitalize Each Word
    ...
    if (!categoryData?.length)
      return { filteredSections: [], categoryLabel: formattedLabel };
    ...
    return { filteredSections: filtered, categoryLabel: formattedLabel };
  }, [categoryData, search, slug]);
  ...
  return (
    <div className="min-h-screen font-sans bg-gradient-to-br from-gray-50 via-white to-gray-50">
      <SEO
        title={
          categoryLabel
            ? `${categoryLabel} Services | Local Trade Street`
            : "Category Services | Local Trade Street"
        }
        description={
          categoryLabel
            ? `Find the best ${categoryLabel} services near you. Browse top subcategories on Local Trade Street.`
            : "Browse top service categories near you on Local Trade Street."
        }
        ogType="website"
      />
