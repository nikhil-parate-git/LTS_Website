  ...
  return (
    ...
    >
      <div className="w-full overflow-hidden relative bg-gray-100" style={{ aspectRatio: "3/2" }}>
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/50 via-black/10 to-transparent opacity-60 group-hover:opacity-90 transition-opacity duration-300" />
        <div className="absolute top-2.5 left-2.5 opacity-0 group-hover:opacity-100 translate-y-1 group-hover:translate-y-0 transition-all duration-300">
          <span className="bg-orange-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wide">View</span>
        </div>
        <div className="absolute bottom-0 left-0 right-0 p-3 translate-y-1 group-hover:translate-y-0 transition-transform duration-300">
          <p className="text-white text-sm font-bold leading-tight drop-shadow-md opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            {item.name}
...
export default function SubCategory() {
  /*
    URL: /subcategory/:cateId/:slug
    cateId = CAT-004 (backend readable ID)
    
    ✅ categories store se cateId match → MongoDB _id nikalo → API call
  */
  ...
  // ✅ cateId (CAT-004) se categories store me match karo → MongoDB _id nikalo
  const categoriesFromStore = useSelector((state) => state.categories?.categories || []);
  const matchedCategory = categoriesFromStore.find((cat) => cat.cateId === cateId);
  const categoryMongoId = matchedCategory?._id || matchedCategory?.id || null;

  useEffect(() => {
    if (categoryMongoId) {
      dispatch(fetchSubcategories(categoryMongoId));
      dispatch(fetchCategoryBanners(categoryMongoId));
    }
  ...
  // cateId se match nahi mila
  if (!categoryMongoId) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 gap-4">
        <div className="w-20 h-20 bg-orange-50 rounded-full flex items-center justify-center text-4xl">🔍</div>
        <h3 className="text-lg font-bold text-gray-700">Category not found</h3>
        <button onClick={() => navigate("/")} className="mt-2 bg-orange-500 text-white px-6 py-2.5 rounded-xl text-sm font-semibold">← Go Home</button>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <MobileSidebarDrawer open={sidebarOpen} onClose={() => setSidebarOpen(false)} cityName={selectedCity} />
        ...
        <div className="flex gap-6">
          <div className="flex-1 min-w-0">
            {loading ? (
              ...
              <div className="text-center py-24 bg-red-50 rounded-3xl border border-red-100">
                <p className="text-red-500 font-bold">Error: {error}</p>
                <button onClick={() => dispatch(fetchSubcategories(categoryMongoId))} className="mt-4 text-orange-500 underline">Try Again</button>
              </div>
            ) : subcategories && subcategories.length > 0 ? (
              <>
                <div className="flex items-center justify-between mb-6 flex-wrap gap-2">
              ...
              <div className="flex flex-col items-center justify-center py-24 text-center">
                ...
                <button onClick={() => navigate(-1)} className="mt-6 bg-orange-500 text-white px-6 py-2.5 rounded-xl text-sm font-semibold">← Go Back</button>
              </div>
            )}
          </div>
