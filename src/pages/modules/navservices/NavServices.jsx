...
const DotLoader = () => (
  <div className="col-span-full flex flex-col items-center justify-center py-20">
    <div className="flex space-x-2">
      <div className="w-4 h-4 bg-orange-600 rounded-full animate-bounce [animation-delay:-0.3s]" />
...
const Pagination = memo(function Pagination({ pagination, onPageChange }) {
  ...
  return (
    <div className="flex items-center justify-center gap-2 mt-10 md:mt-14 flex-wrap">
      <button
        onClick={() => onPageChange(page - 1)}
        disabled={page === 1}
        className="flex items-center gap-1 px-4 py-2 rounded-xl bg-white border border-gray-200 text-gray-600 font-bold text-sm shadow-sm hover:border-orange-500 hover:text-orange-600 transition-all disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:border-gray-200 disabled:hover:text-gray-600"
      >
      ...
      {pages.map((p) => (
        <button
          ...
          className={`w-10 h-10 rounded-xl font-black text-sm transition-all shadow-sm border ${
            p === page
              ? "bg-orange-600 text-white border-orange-600 shadow-orange-200 shadow-lg scale-105"
...
// ─── ServiceCard ───────────────────────────────────────────────────────────────
const ServiceCard = memo(function ServiceCard({ service, onOpenModal }) {
  const handleClick = useCallback(
    () => onOpenModal(service.title),
    [service.title, onOpenModal],
  );

  return (
    <div className="bg-white rounded-2xl md:rounded-3xl shadow-xl shadow-gray-200/50 overflow-hidden border border-gray-100 flex flex-col group transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl">
...
const NavServices = () => {
  ...
  const { items, loading, pagination } = useSelector((state) => state.services);

  useEffect(() => {
    dispatch(fetchServices(currentPage));
  ...
  const handleContactExperts = useCallback(
    () => openModal("General Platform Inquiry"),
    [openModal],
  );
  ...
  return (
    <div className="min-h-screen bg-[#F9FAFB] text-gray-800 font-sans">
      ...
      <div className="container mx-auto px-4 sm:px-6 -mt-10 md:-mt-12 relative z-10 pb-10">
