...
const plans = [
  ...
  },
  {
    ...
    featured: false,
    durations: [
      { label: "12 Months", leads: "unlimited", base: 109999, gst: 19800, total: 129799 },
    ],
    color: "from-indigo-500 to-purple-500",
    dotColor: "bg-indigo-500",
    textAccent: "text-indigo-600",
  ...
  return (
    <div className="relative flex flex-col bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-md transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl">
      ...
      <div className="p-7 pt-10 flex flex-col flex-1">
        ...
        <p className="text-slate-500 text-[13px] leading-relaxed mb-6">
          Get your business verified and listed on the platform. Required before any lead plan.
        </p>
        <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 mb-6 flex-1">
          <div className="text-[11px] text-slate-400 font-medium mb-1">One-time · Incl. GST</div>
          <div className="text-4xl font-black tracking-tight text-slate-800 mb-4">
...
function PlanCard({ plan, durIdx, onDurChange, stepNum }) {
  ...
  return (
    <div className={`relative flex flex-col bg-white border rounded-2xl overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl ${plan.featured ? "border-amber-300 shadow-xl shadow-amber-400/10" : "border-slate-200 shadow-md"}`}>
      <div className={`h-1.5 w-full bg-gradient-to-r ${plan.color}`} />
      ...
      <div className="p-7 pt-10 flex flex-col flex-1">
        ...
        <div className="mb-5">
          <div className="text-[10px] font-semibold tracking-widest uppercase text-slate-400 mb-2">Select Duration</div>
          <div className={`${plan.id === "platinum" ? "flex" : "grid grid-cols-3"} gap-1.5 bg-slate-100 rounded-xl p-1`}>
            {plan.durations.map((d, i) => {
              // Diamond highlight only when sync index is 2 (12 Months)
              const isActive = plan.id === "platinum" ? (durIdx === 2) : (i === durIdx);
        ...
        <div className="mb-6">
          ...
          <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
            <div className={`h-full rounded-full transition-all duration-500 ${plan.barColor}`} style={{ width: `${leadsPct}%` }} />
          </div>
        </div>

        <button className={`w-full py-3.5 rounded-xl text-[13px] font-extrabold tracking-widest uppercase transition-all duration-200 ${plan.featured ? "bg-amber-400 text-white hover:bg-amber-500 shadow-lg shadow-amber-400/30" : "bg-slate-900 text-white hover:bg-slate-700"}`}>
          Get {plan.label} Plan →
        </button>
